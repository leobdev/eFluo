...
namespace eFluo.Services
{
    public class PSNotificationService : IPSNotificationService
    {
        ...
        public PSNotificationService(ApplicationDbContext context, IEmailSender emailSender, IPSRolesService rolesService)
        {
            _context = context;
        ...
        {
            PSUser btUser = await _context.Users.FirstOrDefaultAsync(u => u.Id == notification.RecipientId);
        ...
        public async Task SendEmailNotificationsByRoleAsync(Notification notification, int companyId, string role)
        {
            try
            {
                List<PSUser> members = await _rolesService.GetUsersInRoleAsync(role, companyId);
        ...
        public async Task SendMembersEmailNotificationsAsync(Notification notification, List<PSUser> members)
        {
            try
            {
                foreach (PSUser btUser in members)
